  ...
  return (
    <nav className="fixed top-0 left-0 right-0 z-50 bg-primary/95 backdrop-blur-sm border-b border-gold/20">
      <div className="container mx-auto px-4 sm:px-6 py-3 sm:py-4 flex items-center justify-between gap-2">
        <a href="/" className="font-display text-xl sm:text-2xl font-bold text-primary-foreground tracking-wide shrink-0">
          Qadrdaan
        </a>

        {/* Desktop nav */}
        <div className="hidden lg:flex items-center gap-4 xl:gap-6 font-body text-sm text-primary-foreground/80">
          {mainLinks.map((l) => (
            <a key={l.href} href={l.href} className="hover:text-accent transition-colors whitespace-nowrap">
              {l.label}
            </a>
          ))}
        ...
        <div className="flex items-center gap-2 sm:gap-3">
          ...
          <div className="hidden lg:flex items-center gap-3">
            {user ? (
              <>
                <a href="/profile" className="flex items-center gap-2 px-3 py-2 text-sm font-body text-primary-foreground/90 hover:text-accent transition-colors">
                  ...
                  <span className="hidden xl:inline">{profile?.display_name || "Profile"}</span>
                </a>
                <button onClick={handleSignOut} className="px-3 py-2 text-sm font-body text-primary-foreground/70 hover:text-primary-foreground transition-colors">
                  Sign Out
                </button>
              </>
            ) : (
              <>
                <a href="/auth" className="px-3 py-2 text-sm font-body text-primary-foreground/90 hover:text-accent transition-colors">
                  Sign In
                </a>
                <a href="/auth" className="px-4 py-2 text-sm font-body font-semibold bg-gradient-gold rounded-lg text-primary shadow-gold hover:opacity-90 transition-opacity">
                  Join Free
                </a>
              </>
            )}
