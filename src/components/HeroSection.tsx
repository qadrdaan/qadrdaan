...
const HeroSection = () => (
  <section className="relative min-h-screen flex items-center justify-center overflow-hidden">
    ...
    />
    <motion.div
      className="absolute top-40 right-20 w-3 h-3 rounded-full bg-secondary/30"
      animate={{ y: [0, -30, 0], opacity: [0.2, 0.6, 0.2] }}
      transition={{ duration: 5, repeat: Infinity, delay: 1 }}
    />
    <motion.div
      className="absolute bottom-40 left-1/4 w-1.5 h-1.5 rounded-full bg-accent/50"
    ...
    <div className="relative z-10 container mx-auto px-6 text-center pt-20">
      ...
      >
        <p className="font-body text-primary font-bold text-sm tracking-[0.3em] uppercase mb-6">
      ...
      >
        Welcome to{" "}
        <span className="text-gradient-brand">qadrdaan</span>
      </motion.h1>
      ...
      >
        {[
          { value: "50K+", label: "Poets", color: "text-primary" },
          { value: "200K+", label: "Poems", color: "text-secondary" },
          { value: "30+", label: "Languages", color: "text-accent" },
        ].map((stat) => (
