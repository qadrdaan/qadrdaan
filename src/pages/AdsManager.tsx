      ...
      <section className="pt-28 pb-20 container mx-auto px-6 max-w-5xl">
        <div className="flex items-center justify-between mb-8">
          ...
          <button
            ...
            className="flex items-center gap-2 px-5 py-2.5 font-body font-semibold bg-secondary text-secondary-foreground rounded-lg hover:opacity-90 transition-opacity"
          >
        ...
        <div className="grid grid-cols-2 md:grid-cols-5 gap-4 mb-8">
          {[
            ...
            { label: "Total Spent", value: `$${totalSpent.toFixed(2)}`, icon: DollarSign, color: "text-destructive" },
            { label: "Impressions", value: totalImpressions.toLocaleString(), icon: Eye, color: "text-secondary" },
        ...
        <Tabs defaultValue="campaigns">
          ...
          <TabsContent value="campaigns">
            {campaigns.length === 0 ? (
              <div className="text-center py-16 bg-card border border-border rounded-2xl">
                ...
                <h3 className="font-display text-lg font-bold text-foreground mb-2">No campaigns yet</h3>
                <p className="font-body text-sm text-muted-foreground mb-4">Create your first ad to reach the Qadrdaan community</p>
                <button onClick={() => navigate("/create-ad")} className="px-5 py-2.5 font-body font-semibold bg-secondary text-secondary-foreground rounded-lg">
                  Create Campaign
                </button>
              </div>
            ) : (
              <div className="space-y-4">
                {campaigns.map((c) => {
                  ...
                  return (
                    <div key={c.id} className="bg-card border border-border rounded-xl p-5 flex flex-col md:flex-row md:items-center gap-4">
                      <div className="flex-1">
                        <div className="flex items-center gap-2 mb-1">
                          ...
                          <span className={`px-2 py-0.5 rounded-full text-xs font-body font-medium ${
                            c.status === "active" ? "bg-accent/20 text-accent" :
                            c.status === "paused" ? "bg-muted text-muted-foreground" :
                            c.status === "pending_review" ? "bg-secondary/20 text-secondary" :
                      ...
                      <div className="flex items-center gap-6 text-sm font-body">
                        <div className="text-center">
                          ...
                          <p className="font-semibold text-foreground">{c.impressions_count}</p>
                        </div>
                        <div className="text-center">
                          <p className="text-muted-foreground text-xs">Clicks</p>
                          <p className="font-semibold text-foreground">{c.clicks_count}</p>
                        </div>
                        <div className="text-center">
                          <p className="text-muted-foreground text-xs">CTR</p>
                          <p className="font-semibold text-foreground">{ctr}%</p>
                        </div>
                        <div className="text-center">
                          <p className="text-muted-foreground text-xs">Spent</p>
                          <p className="font-semibold text-foreground">${c.total_spent.toFixed(2)}</p>
                        </div>
                      </div>
                      <div className="flex items-center gap-2">
                        {(c.status === "active" || c.status === "paused") && (
                          <button onClick={() => toggleCampaign(c.id, c.status)} className="p-2 rounded-lg hover:bg-muted transition-colors text-muted-foreground hover:text-foreground">
                            {c.status === "active" ? <Pause className="w-4 h-4" /> : <Play className="w-4 h-4" />}
                          </button>
                        )}
                        {c.status === "draft" && (
                          <button onClick={() => deleteCampaign(c.id)} className="p-2 rounded-lg hover:bg-destructive/10 transition-colors text-muted-foreground hover:text-destructive">
          ...
          <TabsContent value="billing">
            <div className="bg-card border border-border rounded-2xl p-6 max-w-md">
              <h3 className="font-display text-lg font-bold text-foreground mb-4">Add Funds</h3>
                ...
                {["5", "10", "25", "50", "100"].map((amt) => (
                  <button
                    ...
                    className={`px-3 py-1.5 rounded-lg text-sm font-body font-medium transition-colors ${
                      depositAmount === amt ? "bg-secondary text-secondary-foreground" : "bg-muted text-muted-foreground hover:bg-muted/80"
                    }`}
                  >
                    ${amt}
                  </button>
              ...
              <div className="flex gap-3">
                <input
                  ...
                  className="flex-1 px-3 py-2 rounded-lg border border-input bg-background font-body text-sm"
                  placeholder="Amount"
                />
                <button
                  onClick={handleDeposit}
                  className="px-5 py-2 font-body font-semibold bg-accent text-accent-foreground rounded-lg hover:opacity-90 transition-opacity"
                >
                  Deposit
                </button>
              </div>
              <p className="font-body text-xs text-muted-foreground mt-3">
                Payment processing via Stripe/PayPal coming soon. Manual deposits approved by admin.
              </p>
            </div>
          </TabsContent>
        </Tabs>
